...
export const edit = async (req: Request, res: Response) => {
  ...
  try {
    const existingUsers = (await db.query("SELECT * from users WHERE username = $1", [newUsername])).rows;
    if (existingUsers.length > 0) throw new Error("Username taken");
    await db.query("UPDATE users SET username = $1, icon_url = $2 WHERE id = $3", [newUsername, iconUrl, id]);
    await db.query("UPDATE room_members SET room_name = $1 WHERE room_name = $2", [newUsername, oldUsername]);
    res.status(200).json({username: newUsername, icon_url: iconUrl});
  } catch (error) {
    res.status(400).json({Error: getErrorMessage(error)});
  }
}